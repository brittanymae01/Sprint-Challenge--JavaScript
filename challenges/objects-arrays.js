...
console.log(dino1.roar(this.sound));
...
const newArrayUni = graduates.map(currentValue => currentValue.university);
const universities = newArrayUni.sort();
...
const unisWithUni = graduates.filter(currentValue => currentValue.university.includes('Uni'));
...
// });
zooAnimals.forEach(currentValue => displayNames.push(`Name: ${currentValue.animal_name}, Scientific: ${currentValue.scientific_name}`));
...
// })
const lowCaseAnimalNames = zooAnimals.map(currentValue => currentValue.animal_name.toLowerCase());
